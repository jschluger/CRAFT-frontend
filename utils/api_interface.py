...
def wiki_get_ranks(k, t):
    r = {'internal_error': True}  # default
    args = {'k': k}
    if t is not None:
        args['t'] = t
    try:
        r = requests.post(data.BACKEND+'/wiki/viewtop', data=args, timeout=5)
    except Exception as e:
        print(
            f'\nfailed to request {data.BACKEND}/wiki/viewtop\tat time {time.time()}\n\tgot exception {e}')
        return
    return r if type(r) == dict else r.json()
...
def wiki_get_convo(i):
    r = {'internal_error': True}  # default
    args = {'id': i}
    try:
        r = requests.post(data.BACKEND+'/wiki/viewconvo', data=args, timeout=5)
    except Exception as e:
        print(
            f'\nfailed to request {data.BACKEND}/wiki/viewconvo\tat time {time.time()}\n\tgot exception {e}')
    return r if type(r) == dict else r.json()
