...
def reddit_convo():
    ...
    try:
        ...
        print(
            f'got exception <{e}> while parsing arg update_score_pos in /convo')
...
def wiki_home():
    ...
    times = api_interface.get_times()
    ranks = api_interface.wiki_get_ranks(k, t)
...
def wiki_convo():
    ...
    convo = api_interface.wiki_get_convo(i)
    if 'internal_error' in convo:
        abort(500)
    for c in convo['convo']:
        c['text'] = c['text'].split('\n')
    ...
    resp = make_response(render_template('convo.html',      # fine for wikipedia
                                         convo=convo['convo'],
                                         parent=convo['parent'],
                                         endings=convo['endings'],
                                         id=convo['id'],
